...
```
                    ...
                    │                                      │
                    │  ┌──────────┐     ┌──────────────┐  │
  Diagnostic   ◄────┤──│  Linux   │     │    MCU       │  │
  Client            │  │  Gateway │◄───►│  STM32U575   │  │
  (TCP:5002)        │  │ (ARM64)  │ USB │  (ARM CM33)  │  │
                    │  │          │ CDC │              │  │
  Log Storage  ◄────┤──│          │ ECM │              │──┤──► Sensors (emulated)
  (filesystem)      │  └──────────┘     └──────────────┘  │    Actuators (emulated)
                    │                                      │
                    └─────────────────────────────────────┘
```
...
```
              ...
              ┌─────│ CONNECT │◄──────────────┐
              │     └────┬────┘               │
              │          │ TCP established,    │ Recovery
              │          │ state synced        │ successful
              │     ┌────▼────┐          ┌────┴────┐
              │     │ NORMAL  │─────────►│DEGRADED │
              │     └────┬────┘ Packet   └────┬────┘
              │          │      loss>5%       │
